...
public class ImplementacionServlet extends HttpServlet {
	...
	public void createImplementacion(HttpServletRequest req,
		...
		boolean firma = false;
		if (normalizador_str.equals(null) || normalizador_str.equals(""))
			normalizador_str = "NO";
		if (firma_str.equals("") || firma_str.equals(null))
			firma_str = "NO";
		if (normalizador_str.equals("SI"))
			normalizador = true;
		if (firma_str.equals("SI"))
			firma = true;
		...
		if (servicio_id != null && !servicio_id.equals("")) {
			...
			Servicio serv = ServicioDao.getInstance().getImplementacionById(
					Long.parseLong(servicio_id));
			imp.setServicio_name(serv.getName());
		}

		if (cliente_id != null && !cliente_id.equals("")) {
			imp.setCliente_id(Long.parseLong(cliente_id));
			Cliente serv = ClienteDao.getInstance().getClientebyId(
		...
		Utils.writeLog(usermail, "Crea", "Implementacion",
				Long.toString(imp.getServicio_id()));
	...
	public void modifImplementacion(HttpServletRequest req,
			HttpServletResponse resp, String usermail)
			throws InterruptedException {
		JSONObject json = new JSONObject();
		...
		boolean normalizador = false;
		boolean firma = false;
		if (normalizador_str == null)
			normalizador_str = "NO";
		if (firma_str == null)
			firma_str = "NO";
		if (normalizador_str.equals("SI"))
			normalizador = true;
		if (firma_str.endsWith("SI"))
			firma = true;
	...
	public void cloneImplementacion(HttpServletRequest req,
			HttpServletResponse resp, String usermail)
			throws InterruptedException {
		JSONObject json = new JSONObject();
	...
	public void generateXLS(HttpServletRequest req, HttpServletResponse resp,
		...
		try {
			...
			ImplementacionDao impDao = ImplementacionDao.getInstance();
			List<Implementacion> implementaciones = impDao
					.getAllImplementaciones();
			...
			for (Implementacion imp : implementaciones) {
				...
				Servicio servicio = servDao.getImplementacionById(imp
						.getServicio_id());
				...
				if (imp.getNormalizador())
					s.addCell(new Label(7, aux, "Si"));
				else
					s.addCell(new Label(7, aux, "No"));
				s.addCell(new Label(8, aux, imp.getReferencia_global()));
				s.addCell(new Label(9, aux, imp.getReferencia_local()));
				if (imp.getFirma_contrato())
					s.addCell(new Label(10, aux, "Si"));
				else
					s.addCell(new Label(10, aux, "No"));
	...
	public void generateXLSserv(HttpServletRequest req,
			HttpServletResponse resp, String usermail) throws ServletException,
			IOException {
		OutputStream out = null;
		try {
			resp.setContentType("application/vnd.ms-excel");
			...
			s.addCell(new Label(4, 0, "FECHA CONTRATACIÓN PRODUCCIÓN",
					cellFormat));
			...
			for (Implementacion imp : implementaciones) {
				...
				if (imp.getNormalizador())
					s.addCell(new Label(6, aux, "Si"));
				else
					s.addCell(new Label(6, aux, "No"));
	...
	public void deleteImplementacion(HttpServletRequest req,
			HttpServletResponse resp, String usermail) throws ServletException,
			IOException {
		...
		Utils.writeLog(usermail, "Borra", "Implementacion",
				Long.toString(imp.getServicio_id()));
		try {
			json.append("success", "true");
			resp.setCharacterEncoding("UTF-8");
	...
	public void restoreImplementacion(HttpServletRequest req,
			HttpServletResponse resp, String usermail) throws ServletException,
			IOException {
